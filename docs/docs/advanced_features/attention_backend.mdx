...
On SM100/SM103 with CUDA 13+, SGLang automatically selects FlashInfer for GDN prefill when the per-phase override is unset, the base linear-attention backend is Triton, recurrent state is BF16, key/value head dimensions are 128, dynamic chunking and page-major KV layout are disabled, and `--chunked-prefill-size` is between 1 and 8192. On SM120 (RTX PRO 6000 Blackwell) with CUDA 13+, the same auto-selection applies using FlashInfer's SM120 chunked-prefill kernel (FP32 or BF16 recurrent state; the kernel's FP32-state requirement is handled internally). Radix caching may be disabled or use `no_buffer`, `extra_buffer`, or `extra_buffer_lazy`; the extra-buffer paths use state checkpoints.

On SM120, FlashInfer GDN target-verify (speculative decoding) is opt-in via `SGLANG_GDN_FLASHINFER_VERIFY_SM120=1` (the default remains Triton until the MTP kernel is validated on SM120).
...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><strong>FlashInfer (CUDA, SM120 / RTX PRO 6000)</strong></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>✅ (explicit <code>--linear-attn-decode-backend flashinfer</code>)</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>✅ (auto-selected with CUDA 13+)</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>opt-in via <code>SGLANG_GDN_FLASHINFER_VERIFY_SM120=1</code>; Triton by default</td>
    </tr>
  </tbody>
</table>
